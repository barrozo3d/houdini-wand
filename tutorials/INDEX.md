...
- **Summary:** Creates a Torus SOP aligned to the Z-axis, scaled to 0.6 radius with a tube radius of 0.025, with rows increased to 16 for resolution and a Subdivide added for smoothness. Animates rotation on the Z axis at `$F * 20` degrees for a fast spin. Uses a sign() function (covered in a prior lesson) to create an oscillating velocity field around the torus surface. Lifts the torus 1 unit off the ground...
